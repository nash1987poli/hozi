...
for d in districts.values():
    ...
    for h, fm in enumerate(FUTURE, start=1):
        ...
        band = round(4 + 3.0*h, 1)            # confidence band widens with horizon
        fc[fm] = {"risk": round(proj, 1),
                  "low": round(clamp((proj-band)/100)*100, 1),
                  "high": round(clamp((proj+band)/100)*100, 1)}
    d["forecast"] = fc
...
def latest(d):  # most recent known month
