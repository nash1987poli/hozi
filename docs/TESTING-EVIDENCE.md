...
| **LLM contradiction-flagging not yet implemented** | `docs/AI-USAGE-NOTE.md` §2; `scripts/generate-briefs.mjs` | The architecture designates a future validation pass in which the LLM will flag apparent contradictions between the model risk score and external IPC classifications. IPC classification data is not currently passed to the LLM prompt. This is a documented design intent, not a current capability. |
